...
function editMan()
{
    global $d, $langadmin, $func, $strUrl, $curPage, $item, $gallery, $affiliateOffers, $type, $com, $act;
    ...
    if (empty($id)) {
        ...
        if (empty($item)) {
            ...
            if ($act != 'copy') {
                ...
                $affiliateOffers = $d->rawQuery("select * from #_product_affiliate where id_product = ? order by priority desc, id asc", array($id));
            }
        }
    }
}
...
function saveMan()
{
    ...
    if ($data) {
        ...
        $data['pros_vi'] = (isset($_POST['data']['pros_vi'])) ? htmlspecialchars($func->sanitize($_POST['data']['pros_vi'])) : '';
        $data['pros_en'] = (isset($_POST['data']['pros_en'])) ? htmlspecialchars($func->sanitize($_POST['data']['pros_en'])) : '';
        $data['cons_vi'] = (isset($_POST['data']['cons_vi'])) ? htmlspecialchars($func->sanitize($_POST['data']['cons_vi'])) : '';
        $data['cons_en'] = (isset($_POST['data']['cons_en'])) ? htmlspecialchars($func->sanitize($_POST['data']['cons_en'])) : '';
        $data['suitable_vi'] = (isset($_POST['data']['suitable_vi'])) ? htmlspecialchars($func->sanitize($_POST['data']['suitable_vi'])) : '';
        $data['suitable_en'] = (isset($_POST['data']['suitable_en'])) ? htmlspecialchars($func->sanitize($_POST['data']['suitable_en'])) : '';
        $data['specifications_vi'] = (isset($_POST['data']['specifications_vi'])) ? htmlspecialchars($func->sanitize($_POST['data']['specifications_vi'], 'iframe')) : '';
        $data['specifications_en'] = (isset($_POST['data']['specifications_en'])) ? htmlspecialchars($func->sanitize($_POST['data']['specifications_en'], 'iframe')) : '';
        $data['review_type'] = (isset($_POST['data']['review_type'])) ? htmlspecialchars($_POST['data']['review_type']) : 'EDITOR_REVIEW';
        $data['is_real_test'] = (!empty($_POST['is_real_test'])) ? 1 : 0;

        $data['review_score'] = (isset($_POST['data']['review_score']) && $_POST['data']['review_score'] != '') ? (float)$_POST['data']['review_score'] : 0.0;
        $data['review_count'] = (isset($_POST['data']['review_count']) && $_POST['data']['review_count'] != '') ? (int)$_POST['data']['review_count'] : 0;
        $data['expert_pros'] = (isset($_POST['data']['expert_pros'])) ? htmlspecialchars($func->sanitize($_POST['data']['expert_pros'])) : $data['pros_vi'];
        $data['expert_cons'] = (isset($_POST['data']['expert_cons'])) ? htmlspecialchars($func->sanitize($_POST['data']['expert_cons'])) : $data['cons_vi'];
        $data['verdict'] = (isset($_POST['data']['verdict'])) ? htmlspecialchars($func->sanitize($_POST['data']['verdict'])) : '';
        $data['specs'] = (isset($_POST['data']['specs'])) ? trim($_POST['data']['specs']) : '';
        $data['best_for'] = (isset($_POST['data']['best_for'])) ? htmlspecialchars($func->sanitize($_POST['data']['best_for'])) : $data['suitable_vi'];
        $data['affiliate_note'] = (isset($_POST['data']['affiliate_note'])) ? htmlspecialchars($func->sanitize($_POST['data']['affiliate_note'])) : '';

        $data['metaindex'] = ($_POST['metaindex']) ? htmlspecialchars($_POST['metaindex']) : "index";
        $data['metaorder'] = ($_POST['metaorder']) ? implode(",", $_POST['metaorder']) : "";
    }
...
/* Helper lưu Affiliate Offers */
function saveAffiliateOffers($targetProductId)
{
    global $d, $func, $config, $type;
    if (empty($targetProductId)) return;

    $affiliateData = (isset($_POST['affiliate'])) ? $_POST['affiliate'] : array();
    $currentOffers = $d->rawQuery("select id from #_product_affiliate where id_product = ?", array($targetProductId));
    $existingAffIds = array();
    if (!empty($currentOffers)) {
        foreach ($currentOffers as $curr) {
            $existingAffIds[] = (int)$curr['id'];
        }
    }

    $keptAffIds = array();
    if (!empty($affiliateData) && is_array($affiliateData)) {
        foreach ($affiliateData as $affItem) {
            $affUrl = isset($affItem['affiliate_url']) ? trim($affItem['affiliate_url']) : '';
            if (empty($affUrl)) continue;

            if (function_exists('isValidAffiliateUrl') && !isValidAffiliateUrl($affUrl)) {
                continue;
            }

            $affRow = array();
            $affRow['id_product'] = $targetProductId;
            $affRow['platform'] = isset($affItem['platform']) ? htmlspecialchars($affItem['platform']) : 'other';
            $affRow['seller_name'] = isset($affItem['seller_name']) ? htmlspecialchars($func->sanitize($affItem['seller_name'])) : '';
            $affRow['affiliate_url'] = $affUrl;
            $affRow['original_url'] = isset($affItem['original_url']) ? trim($affItem['original_url']) : '';
            $affRow['price'] = isset($affItem['price']) ? (double)str_replace(',', '', $affItem['price']) : 0;
            $affRow['original_price'] = isset($affItem['original_price']) ? (double)str_replace(',', '', $affItem['original_price']) : 0;
            $affRow['coupon_code'] = isset($affItem['coupon_code']) ? htmlspecialchars(trim($affItem['coupon_code'])) : '';
            $affRow['is_best_deal'] = !empty($affItem['is_best_deal']) ? 1 : 0;
            $affRow['commission_rate'] = isset($affItem['commission_rate']) ? (double)$affItem['commission_rate'] : 0;
            $affRow['commission_value'] = isset($affItem['commission_value']) ? (double)str_replace(',', '', $affItem['commission_value']) : 0;
            $affRow['priority'] = isset($affItem['priority']) ? (int)$affItem['priority'] : 0;
            $affRow['status'] = (!empty($affItem['status'])) ? 'hienthi' : 'tamngung';
            $affRow['date_updated'] = time();

            $affId = !empty($affItem['id']) ? (int)$affItem['id'] : 0;
            if ($affId > 0 && in_array($affId, $existingAffIds)) {
                $d->where('id', $affId);
                $d->update('product_affiliate', $affRow);
                $keptAffIds[] = $affId;
            } else {
                $affRow['date_created'] = time();
                $d->insert('product_affiliate', $affRow);
                $newAffId = $d->getLastInsertId();
                if ($newAffId) $keptAffIds[] = $newAffId;
            }
        }
    }

    if (!empty($existingAffIds)) {
        $deleteAffIds = array_diff($existingAffIds, $keptAffIds);
        if (!empty($deleteAffIds)) {
            $deleteStr = implode(',', array_map('intval', $deleteAffIds));
            $d->rawQuery("delete from #_product_affiliate where id in ($deleteStr) and id_product = ?", array($targetProductId));
        }
    }
}
...
function deleteMan()
{
    ...
    if ($id) {
        ...
        if (!empty($row)) {
            ...
            /* Xóa Affiliate & Click logs */
            $d->rawQuery("delete from #_product_affiliate where id_product = ?", array($id));
            $d->rawQuery("delete from #_affiliate_click where id_product = ?", array($id));
