...
class Functions
{
    ...
    public function renderProductBox($products) {
        $output = '<div class="owl-page owl-carousel owl-theme" data-items="screen:0|items:2|margin:10,screen:425|items:2|margin:10,screen:575|items:2|margin:10,screen:767|items:3|margin:10,screen:991|items:4|margin:20,screen:1199|items:4|margin:20" data-rewind="1" data-autoplay="1" data-loop="0" data-lazyload="0" data-mousedrag="1" data-touchdrag="1" data-smartspeed="300" data-autoplayspeed="500" data-autoplaytimeout="3500" data-dots="0" data-nav="0" data-navcontainer="">';
        foreach ($products as $v) {
            $output .= '<div class="box-product">';
            $output .= '<div class="pic-product">';
            $output .= '<a class="text-decoration-none scale-img" href="'.$v[$sluglang] .'" title="'. $v['name' . $lang] .'">
                            <img class="lazy w-100" data-src="'. WATERMARK .'/product/285x285x2/'. UPLOAD_PRODUCT_L . $v['photo'] .'"  />
                        </a>';
            $output .= '</div>';
            $output .= '<h3>' . $v['namevi'] . '</h3>';
            $output .= '</div>';
        }
        $output .= '</div>';
        return $output;
...
'script' => '@<script[^>]*?>.*?</script>@si',
    ...
    public function buildSchemaArticle($id_news, $name, $image, $ngaytao, $ngaysua, $name_author, $url, $logo, $url_author)
    {
        ...
        $str .= '}';
        $str .= '},';
        $str .= '"datePublished": "' . $ngaytao . '",';
        $str .= '"dateModified": "' . $ngaysua . '",';
        $str .= '"author":';
        $str .= '{';
        $str .= '"@type": "Organization",';
        $str .= '"name": "' . $name_author . '",';
